...
contract PerpetuExTest is Test, IPerpetuEx {
    ...
    function shareCalculation(
        uint256 assets
    ) public view returns (uint256 withdrawShares) {
        withdrawShares = Math.mulDiv(
            assets,
            perpetuEx.totalSupply() + 10 ** 0,
            perpetuEx.totalAssets() + 1,
            Math.Rounding.Down
        );
    }
    ...
    function testWithdrawCollateralOpenPositionExists()
        public
    ...
    function testWithdraw() public addLiquidity(LIQUIDITY) {
        ...
        uint256 maxLiquidityToWithdraw = (perpetuEx
            .getTotalLiquidityDeposited() *
            perpetuEx.getMaxUtilizationPercentage()) /
            perpetuEx.getMaxUtilizationPercentageDecimals();
        ...
        vm.stopPrank();
        assertEq(
            perpetuEx.totalAssets(),
            perpetuEx.getTotalLiquidityDeposited()
        );
        assertEq(perpetuEx.totalSupply(), allAssets - withdrawShares);
        assertEq(IERC20(usdc).balanceOf(LP), maxLiquidityToWithdraw);
        assertEq(
            IERC20(perpetuEx).balanceOf(LP),
            allAssets - withdrawShares - DEAD_SHARES
        );
    }
    ...
    function testRedeem() public addLiquidity(LIQUIDITY) {
        ...
        uint256 maxRedeemable = (lpShares *
            perpetuEx.getMaxUtilizationPercentage()) /
            perpetuEx.getMaxUtilizationPercentageDecimals();
        ...
        assertEq(
            allAssets,
            perpetuEx.getTotalLiquidityDeposited() +
                IERC20(usdc).balanceOf(address(LP))
        );
        assertEq(perpetuEx.totalSupply(), allSupply - maxRedeemable);
        assertEq(
            IERC20(usdc).balanceOf(LP),
            allAssets - IERC20(usdc).balanceOf(address(perpetuEx))
        );
        assertEq(
            IERC20(perpetuEx).balanceOf(LP),
            allSupply - maxRedeemable - DEAD_SHARES
        );
    }
    ...
    function testCreateLongPosition()
        public
        ...
        addCollateral(COLLATERAL)
    {
        ...
        (bool isLong, uint256 totalValue, uint256 size, , , ) = perpetuEx
            .positions(positionId);

        assert(isLong);
        assertEq(perpetuEx.collateral(USER), COLLATERAL);
        assertEq(size, SIZE);
        uint256 longOpenInterestInTokens = perpetuEx
    ...
    function testCreateShortPosition()
        public
        addLiquidity(LIQUIDITY)
        addCollateral(COLLATERAL)
    {
    ...
    function testOpenLongAndShortPositions()
        public
        addLiquidity(LIQUIDITY)
        addCollateral(COLLATERAL)
    {
        ...
        (bool isLong, uint256 totalValue, uint256 size, , , ) = perpetuEx
            .positions(positionIdUser);
        assert(isLong);
        assertEq(perpetuEx.collateral(USER), COLLATERAL);
        assertEq(size, SIZE);
        uint256 averageOpenPriceUser = perpetuEx.getAverageOpenPrice(
            positionIdUser
        );
        ...
        (bool isLong2, uint256 totalValue2, uint256 size2, , , ) = perpetuEx
            .positions(positionIdUser2);
        assert(!isLong2);
        assertEq(perpetuEx.collateral(USER2), COLLATERAL);
        assertEq(size2, SIZE_2);
        uint256 averageOpenPriceUser2 = perpetuEx.getAverageOpenPrice(
            positionIdUser2
        );
        ...
        depositCollateralOpenLongPosition(COLLATERAL)
    {
        ...
        vm.stopPrank();
        (, uint256 totalValue, uint256 size, , , ) = perpetuEx.positions(
            positionId
        );
    ...
    function testDecreaseCollateral()
        public
        longPositionOpened(LIQUIDITY, COLLATERAL, SIZE)
    {
        vm.startPrank(USER);
    ...
    function testBorrowingFeesForAYear()
        public
        longPositionOpened(LIQUIDITY, COLLATERAL, SIZE)
    {
        // fees after 1 year
    ...
    function testLiquidateNoLiquidationNeeded()
        public
        longPositionOpened(LIQUIDITY, COLLATERAL, SIZE)
    {
        ...
        longPositionOpened(LIQUIDITY, COLLATERAL, SIZE)
    {
        ...
        perpetuEx.liquidate(USER);
        vm.stopPrank();
        (, , uint256 size, uint256 collateral, , ) = perpetuEx.positions(
            positionId
        );
        ...
        assertEq(
            perpetuEx.getTotalLiquidityDeposited(),
            totalLiquidityDepositedBefore + backToProtocol
        );
    }
        ...
        longPositionOpened(LIQUIDITY, COLLATERAL, SIZE)
    {
        ...
        uint256 expectedUserLeverage = (SIZE * currentPrice) /
            (COLLATERAL * 1e9);
