...
contract PerpetuExTest is Test, IPerpetuEx {
    ...
    function shareCalculation(uint256 assets) public view returns (uint256 withdrawShares) {
        withdrawShares =
            Math.mulDiv(assets, perpetuEx.totalSupply() + 10 ** 0, perpetuEx.totalAssets() + 1, Math.Rounding.Down);
    }
    ...
    function testWithdraw() public addLiquidity(LIQUIDITY) {
        ...
        uint256 maxLiquidityToWithdraw = (
            perpetuEx.getTotalLiquidityDeposited() * perpetuEx.getMaxUtilizationPercentage()
